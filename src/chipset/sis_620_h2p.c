 ...
 *
 *          Implementation of the SiS 620 Host to PCI bridge.
 *
 * Authors: Dareka5555
 *
 *          Copyright 2026 Dareka5555
 */
#include <stdarg.h>
#include <stdio.h>
#include <stdint.h>
...
#ifdef ENABLE_SIS_620_HOST_TO_PCI_LOG
int sis_620_host_to_pci_do_log = ENABLE_SIS_620_HOST_TO_PCI_LOG;

static void
sis_620_host_to_pci_log(const char *fmt, ...)
{
    va_list ap;

    if (sis_620_host_to_pci_do_log) {
        va_start(ap, fmt);
        pclog_ex(fmt, ap);
        va_end(ap);
    }
...
#else
#    define sis_620_host_to_pci_log(fmt, ...)
#endif
...
typedef struct sis_620_host_to_pci_t {
...
static void
sis_620_shadow_recalc(sis_620_host_to_pci_t *dev)
{
    int      state;
    uint32_t base;
    ...
    for (uint8_t i = 0; i < 8; i++) {
        ...
        if (((dev->pci_conf[0x70] ^ dev->states[0]) & (1 << i)) ||
            ...
            sis_620_host_to_pci_log("%08X-%08X\n", base, base + 0x3fff);
        }
    }
...
static void
sis_620_smram_recalc(sis_620_host_to_pci_t *dev)
{
    smram_disable_all();
...
static void
sis_620_mask_bar(uint8_t *regs, void *agpgart)
{
    uint32_t bar;
    uint32_t size;
    ...
    size = sizes[(regs[0x94] >> 4) & 0x07];

    /* Make sure the aperture's base is aligned to its size. */
    bar = regs[0x10] | (regs[0x11] << 8) | (regs[0x12] << 16) | (regs[0x13] << 24);
    if (size != 0x00000000)
        bar &= ~(size - 1);
    regs[0x10] = bar & 0xff;
    regs[0x11] = (bar >> 8) & 0xff;
...
void
sis_620_host_to_pci_write(int addr, uint8_t val, void *priv)
{
    sis_620_host_to_pci_t *dev = (sis_620_host_to_pci_t *) priv;

    sis_620_host_to_pci_log("SiS 620 H2P: [W] dev->pci_conf[%02X] = %02X\n", addr, val);
    ...
    switch (addr) {
        ...
        /* Graphic Window Base Address - 32-bit BAR. */
        case 0x10 ... 0x13:
            dev->pci_conf[addr] = val;
            sis_620_mask_bar(dev->pci_conf, dev->agpgart);
            break;
        ...
        case 0x6a:
            dev->pci_conf[addr] = val;
            sis_620_smram_recalc(dev);
            break;

        case 0x70 ... 0x73:
            dev->pci_conf[addr] = val;
            sis_620_shadow_recalc(dev);
            break;
        ...
        /* Page Table Cache Invalidation Control - bit 1 is self-clearing, so
           it must never be readable as set. */
        case 0x98:
...
uint8_t
sis_620_host_to_pci_read(int addr, void *priv)
{
    const sis_620_host_to_pci_t *dev = (sis_620_host_to_pci_t *) priv;
    uint8_t ret = 0xff;

    ret = dev->pci_conf[addr];

    sis_620_host_to_pci_log("SiS 620 H2P: [R] dev->pci_conf[%02X] = %02X\n", addr, ret);

    return ret;
}

static void
sis_620_host_to_pci_reset(void *priv)
{
    sis_620_host_to_pci_t *dev = (sis_620_host_to_pci_t *) priv;

    dev->pci_conf[0x00] = 0x39;
    dev->pci_conf[0x01] = 0x10;
    dev->pci_conf[0x02] = 0x20;
    dev->pci_conf[0x03] = 0x06;
...
static void
sis_620_host_to_pci_close(void *priv)
{
    sis_620_host_to_pci_t *dev = (sis_620_host_to_pci_t *) priv;
...
static void *
sis_620_host_to_pci_init(UNUSED(const device_t *info))
{
    sis_620_host_to_pci_t *dev = (sis_620_host_to_pci_t *) calloc(1, sizeof(sis_620_host_to_pci_t));
    ...
    sis_620_host_to_pci_reset(dev);

    return dev;
}

const device_t sis_620_h2p_device = {
    .name          = "SiS 620 Host to PCI bridge",
    .internal_name = "sis_620_host_to_pci",
    .flags         = DEVICE_PCI,
    .local         = 0x00,
    .init          = sis_620_host_to_pci_init,
    .close         = sis_620_host_to_pci_close,
    .reset         = sis_620_host_to_pci_reset,
    .available     = NULL,
    .speed_changed = NULL,
    .force_redraw  = NULL,
    .config        = NULL
